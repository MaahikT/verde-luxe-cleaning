...
export const getUpcomingCardHolds = baseProcedure
  .input(
    z.object({
      authToken: z.string(),
  ...
  .query(async ({ input }) => {
    try {
      ...
      if (user.role === "ADMIN" && !hasPermission(user, "manage_bookings")) {
        throw new TRPCError({
          code: "FORBIDDEN",
          message: "You do not have permission to view card holds",
        });
      }
      ...
      const bookingWhere: any = {
        AND: [
          {
            // Only get upcoming bookings (not completed, not cancelled, and scheduled in future or pending/confirmed)
            OR: [
              {
      ...
      // Fetch bookings with upcoming card holds
